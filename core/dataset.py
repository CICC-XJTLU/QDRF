...
class MMDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        self.token_length = args['model']['feature_extractor']['input_length'][0]
    ...
    def __init_mosi(self):
        ...
        if self.mode == 'train':
            # Generate a random matrix uniformly distributed within the interval of [0,1), with each sample having a random missing_rate.
            ...
            if isinstance(self.missing_rate_eval_test, dict):
    ...
    def __getitem__(self, index):
        ...
        sample = {
            ...
            'mask_v': torch.Tensor(self.vision_missing_mask[index]),
            'mask_a': torch.Tensor(self.audio_missing_mask[index]),
            'mask_t': torch.Tensor(self.text_missing_mask[index]),
            'index': index,
            'id': self.ids[index],
